...
def parse_arguments():
    ...
    parser.add_argument('--maxconn', type=int, default=4,
                        help='Maximum number of database connections')
    parser.add_argument('--num_chunks', type=int, default=4,
                        help='Number of chunks to split the dataframe into')
...
def update_value(row):
    ...
    if existing_value != existing_value:
        ...
        if isinstance(new_value, (str, list, np.ndarray)):
            return (
                new_value.tolist() if isinstance(new_value, np.ndarray)
                else new_value, 'INSERT', True
            )
        else:
...
def process_chunk(chunk, result_queue):
    ...
    try:
        ...
        cur.execute(f"SELECT {id_col}, {value_col} FROM {table_name} WHERE {id_col} IN %s",
                    (tuple(order_ids),))
...
def start_receiver(args):
    ...
    channel.basic_consume(
        queue='dataframe_queue', on_message_callback=on_message_callback
    )
    channel.basic_consume(
        queue='control_queue', on_message_callback=process_control_message
    )
